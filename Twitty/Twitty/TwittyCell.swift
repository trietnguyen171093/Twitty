...
class TwittyCell: UITableViewCell {
  ...
  var hometimeline : TimeLine!{
    didSet{
      ...
      // calculate time ago
      let createdTimeString =  hometimeline.timecreate
      var createdTime: NSDate?
      if let createTimeString = createdTimeString {
        let dateformat = DateFormatter()
        dateformat.dateFormat = "EEE MMM d HH:mm:ss Z y"
        createdTime = dateformat.date(from: createTimeString)!
          as NSDate
      }
      timeLabel.text = timeAgoSinceDate(date: createdTime!, numericDates: true)
